...
from tools.bybit_playground_helpers.history_exports import export_bybit_history_excels

# ORDER_STATE_TABLE = "order_state"

PLAYGROUND_API_KEY_ENV = "PLAYGROUND_BYBIT_API_KEY"
PLAYGROUND_API_SECRET_ENV = "PLAYGROUND_BYBIT_API_SECRET"
PLAYGROUND_TESTNET_ENV = "PLAYGROUND_BYBIT_TESTNET"
...
def _load_playground_creds_from_env() -> ExchangeCredentials | None:
    api_key = os.getenv(PLAYGROUND_API_KEY_ENV)
    api_secret = os.getenv(PLAYGROUND_API_SECRET_ENV)
    if not api_key or not api_secret:
        return None
    testnet_raw = os.getenv(PLAYGROUND_TESTNET_ENV, "")
    testnet = str(testnet_raw).strip().lower() in {"1", "true", "yes", "y", "on"}
...
def _load_first_row_and_creds():
    env_creds = _load_playground_creds_from_env()
    if env_creds:
        return {"exchange": "bybit", "source": "env"}, env_creds
...
def main() -> None:
    ...
    resp = api.get_ads_list(page="1", size="3")
    items = resp.get("result", {}).get("items", [])
    if items:
        pprint(items[0].get("tradingPreferenceSet"))
